...
if (!isset($_GET['dir'])) {
      file_put_contents("scripts/subfolder.php", "repository");
      $subFolder = "ds-none";
      $getFile = "SELECT * FROM repository WHERE file_directory = :parentFolder";
      $stmts = $conn->prepare($getFile);
      $result = $stmts->execute([
        'parentFolder' => "repository"
      ]);
} else {
    $subDirection = $_GET['dir'];
    file_put_contents("scripts/subfolder.php", $_GET['dir']);
    $subFolder = "";
    $getFile = "SELECT * FROM repository WHERE file_directory = :fileName";
    $stmts = $conn->prepare($getFile);
    $stmts->execute([
      "fileName" => $_GET['dir']
    ]);
      ...
      <div class="file-meta sw-mt-2 sw-grid-row">
        <div class="sw-grid-half">
          <ul class="sw-breadcrumb-i font-xm sw-rsq-2 bg-trans">
            <li class="sw-breadcrumb-list"><a href="index.php" class="st-coral"><i class="fa fa-home"></i>Home</a></li>
            <?php
            ...
            if (!isset($_GET['dir'])) {
              ...
              echo '
          <li class="sw-breadcrumb-list active st-dodgerblue"><i class="st-goldenrod fa fa-folder-open"></i>&nbsp;'.$subDirection.'</li>
                ';
            }
...
while ($row = $stmts->fetch(PDO::FETCH_ASSOC)) {
       ...
       if (!isset($_GET['dir'])) {
          if ($fileType == "folder") {
          $dir = "https://localhost/file-manager?dir=$fileName";
          $target = "";
          $fileSize = $fileSizeUnit = "";
          $countContent = '<span class="sw-badge bg-coral sw-rsq-2 font-xl">'.$fileContent.'</span>';
        } else {
          $dir = "https://localhost/file-manager/repository/$fileName";
          $target = "_blank";
          $countContent = '';
        }
       } else {
        ...
        } else if ($fileType == "application/vnd.openxmlformats-officedocument.presentationml.presentation" || $fileType == "pptx") {
          $fileType = "PPT FILE";
          $fileIcon = "st-red fa fa-file-powerpoint-o";
        } else if ($fileType == "application/x-zip-compressed" || $fileType == "zip") {
          $fileType = "PPT FILE";
          $fileIcon = "st-goldenrod fa fa-file-zip-o";
        } else if ($fileType == "audio/mpeg" || $fileType == "mpeg" || $fileType == "mp3") {
          $fileType = "AUDIO FILE";
          $fileIcon = "st-turquoise fa fa-music";
        } else if ($fileType == "application/vnd.ms-excel.sheet.macroEnabled.12" || $fileType == "application/vnd.ms-excel.sheet" || $fileType == "xls" || $fileType == "xlsm") {
          $fileType = "XLS FILE";
          $fileIcon = "st-green fa fa-file-excel-o";
        } else if ($fileType == "image/jpeg" || $fileType == "image/jpg" || $fileType == "image/png" || $fileType == "image/gif" || $fileType == "jpg" || $fileType == "jpeg" || $fileType == "png" || $fileType == "gif") {
          $fileType = "IMAGE FILE";
...
<td><a href="<?=$dir;?>" target="<?=$target;?>" class="filename st-black"><i class="<?=$fileIcon;?> font-sw"></i>&nbsp;<?=$fileName;?>&nbsp;<?=$countContent;?></a></td>
<td><?=$fileType;?></td>
...
<td width="10"><button class="rename sw-btn"><i class="fa fa-pencil"></i></button></td>
</tr>
<?php }} ?>
</tbody>
</table>
