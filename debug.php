...
if ($countFiles == 0) {
  ...
  <td colspan="6" class="st-center">NO FILES YET</td>
    ...
    $selectFolder = "SELECT * FROM repository WHERE file_name = :fileName";
    $stmtFolder = $conn->prepare($selectFolder);
    $stmtFolder->execute([
      "fileName" => $_GET['dir']
    ]);
    $rowFolder = $stmtFolder->fetch();
    $fileContent = $rowFolder["file_content"];
